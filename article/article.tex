...
The analysis presented in this paper was based upon a dataset consisting of the 49 nuclear receptors of mouse~\cite{proteomic} and their associated genes (for a full list of gene names, please see Appendix~\ref{an:appendix}). The gene names were further on used to compile additional information regarding the position, associated SNPs and phenotypes for the nuclear receptors in mice.

\subsection{MGI - Mouse Genome Informatics}

Mouse Genome Informatics\footnote{\url{http://www.informatics.jax.org/}, \today}~\cite{mgi} is free, online database for the laboratory mouse, which provides access to information about integrated genetics and associations between specific phenotypes and their corresponding alleles.
~~~~~~~\\
~~~~~~~\\
For this research, the MGI database was solely used for building a connection between the nuclear receptor genes in the mouse and the associated phenotypes dependent on miscellaneous strains. Information about the SNPs and the variation of the nuclear receptors were not taken from MGI, because the annotation regarding this matter was far better in the MPI and UCSC database.
...
The Mouse Genome Database contains over 24000 genes with protein sequences and approximately 48000 genotypes with phenotype annotation ~\cite{mgd}.
For this paper the MGD was solely used for building a connection between the nuclear receptor genes and the associated phenotypes always dependent on miscellaneous strains. Information about SNPs and variation of the nuclear receptors were not taken from MGD, because the annotation regarding this matter was far better in the MPI and UCSC database.
\subsection{MPD}
...
More than 1330 strains were examined in several ways, over 3500 phenotype and 1.8 billion genotype annotations were made~\cite{mpd}.
...
The goal of the database was to create several tables which could be mapped with the reference table of the 49 nuclear receptors, to create statistics about the relationship between the nuclear receptors and the phenotypes and SNPs associated with them. As mentioned above, the main problem was the connection between the MGD and the MPD tables, which was gone around by using an extra table which contained only the strains of MGI which are the same as the ones of the MPD table. As additional information regarding the SNPs another database was consulted, the UCSC database. This table contains every SNP of the MPD table, but with infromation about the reference genome and the locations of the SNPs. Having that information, it is possible to create statistics regarding the substitutions of the nucleotides, locations and the phenotypic results arising from those SNPs. 
...
All databases were build upon the \textit{nr\_mapping} table, which contains the important association between the nuclear receptors and the gene names. The \textit{mgi} table with the information regarding the genes itself like allele attributes and types was connected with the \textit{mgi\_phenotypes} table which stored the phenotypic information in association with the internal MGD ID. To map the nuclear receptors and the data of MGD on MPD we used the \textit{mgi\_strains} table which contained every available strain in mgi and built with that and the gene names of \textit{mgi} the \textit{mpi\_strainmeans} table. This table stores information about experimental results from strains like means, maximum values, minimum value and the zscore which will be an important indicator in the results chapter. Further the table includes an \textit{measurementsnumber} which was the key identifier for an element in the \textit{mpi\_strainmeans} and the \textit{mpi\_measnum} table which stores information about phenotypes associated with the regarded genes. To get a more view on the genotypic aspect besides the phenotypic, the \textit{mpi\_rs} and the \textit{ucsc\_variants} tables were built. They store the information about SNPs in the genes which are affected by the nuclear receptors. 
Given that the table for MGD was created by searching the gene names in the database and extracting all necessary information, analogous for the MPD database.
%------------------------------------------------
...
\hline
	\textbf{Strain} & \textbf{Gene}  & \textbf{Feature} & \textbf{Phenotype} & \textbf{Z-score} & \textbf{Occurrence} \\  
	\hline \hline 
	CAST/EiJ  & Rora (RAR-related orphan receptor alpha) & protein coding gene & trigonelline relative abundance &  8.870 & 2440\\ 
	C58/J  & Rorc (RAR-related orphan receptor gamma) & protein coding gene & N-acetylglutamate relative abundance & 8.450 & 1398\\ 
	BUB/BnJ &  & 3-dehydrocarnitine  relative abundance &  8.420 & 1716\\ 
	NOD/ShiLtJ   &  & glucose &  7.840 & 2599\\ 
	NZW/LacJ   &  & glycerate  relative abundance &  7.410 & 2230\\ 
	FVB/NJ  &  & succinylcarnitine  relative abundance  &  6.450 & 3335\\ 
	NZB/BlNJ  &  & cornea-lens synechia  &  6.250 & 1342\\ 
	SEA/GnJ  &  & optic cup changes &  6.170 & 912\\ 
	NZO/HlLtJ &  &  body weight &  6.130 & 1007\\ 
	129S1/SvImJ  &  & latency to reach periphery 10 min test  &  6.080 & 3360\\ 
	\hline
\end{tabulary}
