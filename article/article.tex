...
\hline
	...
	CE/J & Rorb (RAR-related orphan receptor beta) & 
	tigloylglycine (16h fast), relative abundance&  3.3\\
...
%------------------------------------------------
\section{Nuclear receptors in the human genome}

In contrast to the nuclear receptors in mouse, there have been two studies about nuclear receptors in human, called \textit{kora} and \textit{twinsuk}. With the results of these studies the database used in this paper was expanded beyond one species with the goal to find similarities between the nuclear receptors in human and mouse. Because of the lack of researched nuclear receptors in mouse there can be only made comparisons regarding the phenotype with the tables \textit{mgi\_phenotypes} \ref{fig:database} and \textit{mpg\_phenotypes} \ref{fig:database}. Table \ref{tab:compHuMo} shows a list with nuclear receptors which are found in mouse and human and have a similar phenotype or a similar biological function.
~~~~~~~\\
\begin{table}[H]
\begin{tabulary}{\linewidth}{ LLLLL }
\hline
	\textbf{Gene} & \textbf{Mouse phenotype}  & \textbf{Human phenotype}
	\\  
	\hline \hline 
...
~~~~~~~\\
Nuclear receptors in human and mouse which share a similar protein or a similar biological function.}} 
\label{tab:compHuMo}
\end{table}
...
The statistics about extreme phenotypes which manifest if a variation in a nuclear receptor in mice occurs, show clearly that very important fields are affected which leads to the conclusion that nuclear receptors have a big influence on the development of the mouse. The data of \textit{MGI} and \textit{MPD} on nuclear receptors is limited, the phenotypes annotated in \textit{MGI} decribe more the biological systems in which the phenotypes occur rather than the description of differences to a non variant like in \textit{MPD}. Nevertheless, in contrast to the human genome where there are less annotations than in the mouse databases, the conclusion on the comparison between the two species shows a shallow match regarding the phenotypic expression in case of a variation in one of the same nuclear receptors.
