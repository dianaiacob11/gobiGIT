...
sub usage{
    ...
    print "Usage: perl analysisMPI_top10_strains_distribution.pl --outputFile <path_to_plot_file> [--d|debug]\n";
    print "       $0 --help \n";
    print "       $0 --man \n";
    exit;
}
...
#extract plot data
my ($strains, $percentage, $count, $total_strains) = MODULES::PlotsExtract::get_analysisMPI_top10_strains_distribution();

#transform plot data
my $x_axis                  = $strains;
my $y_axis                  = $percentage;
my $xlab                    = "Strains";
my $ylab                    = "Strain frequency and z-score for nuclear receptors";
my $legend_count_strains    = "Strain occurrences: ".$total_strains;
my $legend_count_nr         = "Nuclear receptors in mouse: 49";
my $label                   = $count;

#generate plots
MODULES::PlotsGenerate::plotVerticalString($x_axis, $y_axis, $outputFile, $xlab, $ylab, $legend_count_strains, $legend_count_nr, $label);
